...
const SearchPage: React.FC = () => {
  ...
  const includeAdult = searchParams.get("includeAdult") || 'no';
    ...
    // console.log(searchParams, title, year)
  const [currentPage, setCurrentPage] = useState(1);
  const {
    data,
    isLoading,
    refetch,
  } = useQuery(["popularMovies", currentPage],() => searchMovies(title,includeAdult,year,currentPage)
  );
    ...
    <Fragment>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <h1 className="text-gray-700 font-extrabold text-2xl my-2">Looking movie "{title}" at year {year} {includeAdult === "yes" ? "and include adult" : ""}</h1>
